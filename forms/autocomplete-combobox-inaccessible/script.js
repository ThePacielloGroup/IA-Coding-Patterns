...
function updateStateList(){
  ...
  for (var i = 0; i < arrayLength; i++) {
    ...
    jQuery('#listOptions').append('<li class="option" tabindex="-1" id="stateId'+matchedStates[i]+'">' + matchedStates[i] + '</li>');
  }
  ...
  jQuery('#suggestCombo').keydown(function(evt) {
    ...
    if (evt.which == 38){
      if(currentSelected == -1){
        jQuery('li.option').eq(totalMatched - 1).addClass("selected");
        
        //jQuery('li[role="option"]').eq(totalMatched - 1).focus();
        currentSelected=totalMatched - 1;
      } else if (currentSelected == 0){
        jQuery('li.option').eq(currentSelected).removeClass("selected");
        ...
        jQuery('li.option').eq(currentSelected).removeClass("selected");
        jQuery('li.option').eq(currentSelected - 1).addClass("selected");
    ...
    if (evt.which == 40){
      if(currentSelected == -1){
        jQuery('li.option').eq(0).addClass("selected");
    ...
    if (evt.which == 13){
      if(currentSelected != -1){
        jQuery('#f').val(jQuery('li.option').eq(currentSelected).text());
        //jQuery('#f').focus();
        jQuery('#listOptions').empty();
        currentSelected = -1;
        jQuery('#states').hide();
